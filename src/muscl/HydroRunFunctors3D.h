...
class ComputeGravityStep3D : public HydroBaseFunctor3D
{
public:
    ComputeGravityStep3D(HydroParams params,
                         DataArray3d Udata,
                         real_t dt) :
    ...
    static void apply(HydroParams params,
                      ...
                      int nbCells)
    {
        ComputeGravityStep3D functor(params, Udata, dt);
    ...
    void operator()(const int& index) const
    {
           ...
           i >= ghostWidth && i < isize - ghostWidth)
        {
            ...
            uLoc[IP] -= HALF_F*(uLoc[IU]*uLoc[IU] + uLoc[IV]*uLoc[IV] + uLoc[IW]*uLoc[IW]) / uLoc[ID];
            uLoc[IU] += dt * uLoc[ID] * params.settings.g_x;
            uLoc[IV] += dt * uLoc[ID] * params.settings.g_y;
            uLoc[IW] += dt * uLoc[ID] * params.settings.g_z;
            uLoc[IP] += HALF_F*(uLoc[IU]*uLoc[IU] + uLoc[IV]*uLoc[IV] + uLoc[IW]*uLoc[IW]) / uLoc[ID];

            Udata(i, j, k, IP) = uLoc[IP];
            Udata(i, j, k, IU) = uLoc[IU];
            Udata(i, j, k, IV) = uLoc[IV];
            Udata(i, j, k, IW) = uLoc[IW];
        }
    }

    DataArray3d Udata;
    const real_t dt;
}; // ComputeGravityStep3D
