...
namespace euler_kokkos { namespace io
	{
				...
				std::string debug_name)
		{
			...
			if (outputVtkAscii) {
						...
						if (iVar==IP)
						{
							for (int j=jmin+ghostWidth; j<=jmax-ghostWidth; j++)
								for (int i=imin+ghostWidth; i<=imax-ghostWidth; i++) {
									const real_t barotropic = Uhost(i, j, IH)>ZERO_F? params.settings.barotropic0:params.settings.barotropic1;
									if (!barotropic)
										outFile << Uhost(i, j, iVar) << " ";
									else
									{
										const real_t rho0=Uhost(i, j, IH)>ZERO_F? params.settings.rho0:params.settings.rho1;
										const real_t c= Uhost(i, j, IH)>ZERO_F? params.settings.sound_speed0:params.settings.sound_speed1;
										const real_t p0= Uhost(i, j, IH)>ZERO_F? params.Astate0:params.Astate1;
										outFile << p0 + (Uhost(i, j, ID)- rho0) * c * c<< " ";
				...
				if (params.settings.kappa0>ZERO_F)
				{
					outFile << "    <DataArray type=\"";
					if (useDouble)
						outFile << "Float64";
					...
					for (int index=0; index<nbCells; ++index) {
						...
						for (int j=jmin+ghostWidth; j<=jmax-ghostWidth; j++)
							for (int i=imin+ghostWidth; i<=imax-ghostWidth; i++) {
								...
								if (!barotropic)
								{
									const real_t Bstate=Uhost(i, j, IH)>ZERO_F? params.Bstate0:params.Bstate1;
									const real_t Rstar=Uhost(i, j, IH)>ZERO_F? params.settings.Rstar0:params.settings.Rstar1;
									outFile << (Uhost(i, j, IP) + Bstate)/Uhost(i, j, ID)/Rstar << " ";
								}
								else
									outFile << Uhost(i, j, IP) << " ";
							}
					}
				} 
			...
			{ // write data in binary format
				...
				if (params.settings.kappa0>ZERO_F)
				{
					...
					outFile << "T"
						<< "\" format=\"appended\" offset=\""
						<< nbvar*nx*ny*sizeof(real_t)+nbvar*sizeof(unsigned int)
						<<"\" />" << std::endl;
				}
						...
						for (int j=jmin+ghostWidth; j<=jmax-ghostWidth; j++)
							for (int i=imin+ghostWidth; i<=imax-ghostWidth; i++) {
								...
								if (!barotropic)
								{
									const real_t Bstate=Uhost(i, j, IH)>ZERO_F? params.Bstate0:params.Bstate1;
									const real_t Rstar=Uhost(i, j, IH)>ZERO_F? params.settings.Rstar0:params.settings.Rstar1;
									tmp = (Uhost(i, j, IP) + Bstate)/Uhost(i, j, ID)/Rstar;
								}
								else
									tmp = Uhost(i, j, IP);
