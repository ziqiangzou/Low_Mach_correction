...
double ConfigMap::getFloat(std::string section, std::string name, double default_value)
{
  std::string valstr = getString(section, name, "");
  const char* value = valstr.c_str();
  char* end;
  // This parses "1234" (decimal) and also "0x4D2" (hex)
  double valFloat = strtod(value, &end);
  return end > value ? valFloat : default_value;
} // ConfigMap::getFloat
long double ConfigMap::getFloat(std::string section, std::string name,long double default_value)
{
  std::string valstr = getString(section, name, "");
  const char* value = valstr.c_str();
  char* end;
  // This parses "1234" (decimal) and also "0x4D2" (hex)
  double valFloat = strtold(value, &end);
  return end > value ? valFloat : default_value;
} // ConfigMap::getFloat
