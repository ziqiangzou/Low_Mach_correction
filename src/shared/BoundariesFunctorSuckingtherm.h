...
namespace euler_kokkos
{
	...
	template <FaceIdType faceId>
		class MakeBoundariesFunctor2D_Suckingtherm : all_regime::HydroBaseFunctor2D
	{
		public:
			MakeBoundariesFunctor2D_Suckingtherm(HydroParams params_,
			...
			static void apply(HydroParams params,
					...
					int nbCells)
			{
				MakeBoundariesFunctor2D_Suckingtherm<faceId> functor(params, suckingthermParams, Udata);
			...
			KOKKOS_INLINE_FUNCTION
				void operator()(const int& index) const
				{
					...
					const real_t T_top = params.settings.Tsat;
