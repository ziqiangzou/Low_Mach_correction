...
namespace euler_kokkos
{
	...
	template <FaceIdType faceId>
		class MakeBoundariesFunctor2D_RisingBubble : all_regime::HydroBaseFunctor2D
	{
		public:
			MakeBoundariesFunctor2D_RisingBubble(HydroParams params_, DataArray2d Udata_) :
			...
			static void apply(HydroParams params,
					...
					int nbCells)
			{
				MakeBoundariesFunctor2D_RisingBubble<faceId> functor(params, Udata);
					...
					if (faceId == FACE_YMIN)
					{
						...
						if(i >= imin && i <= imax)
						{
							for (int j=jmin+ghostWidth-1; j>=jmin; --j)
							{
								...
								const real_t phiLoc = Udata(i, j0, IH); 
								...
								const real_t p_R = ((gamma - ONE_F) *(Udata(i, j0, IP) - HALF_F * rho_R * (u_R * u_R + v_R * v_R)))-gamma* Bstate;
								...
								Udata(i, j, IH) = phiLoc;
								Udata(i, j, IP) = (p_L+gamma*Bstate)/(gamma-ONE_F) + HALF_F * rho_L * (u_L * u_L + v_L * v_L);
					...
					if (faceId == FACE_YMAX)
					{
						...
						if(i >= imin && i <= imax)
						{
							for (int j=ny+ghostWidth; j<=ny+2*ghostWidth-1; ++j)
							{
								...
								const real_t p_L = ((gamma - ONE_F) *(Udata(i, j0, IP) - HALF_F * rho_L * (u_L * u_L + v_L * v_L)))-gamma* Bstate;
								...
								Udata(i, j, IH) = phiLoc;
								Udata(i, j, IP) = (p_R+gamma*Bstate)/(gamma-ONE_F) + HALF_F * rho_R * (u_R * u_R + v_R * v_R);
							}
						}
					}
				}
	...
	template <FaceIdType faceId>
		class MakeBoundariesFunctor3D_RisingBubble : all_regime::HydroBaseFunctor3D
	{
		public:
			MakeBoundariesFunctor3D_RisingBubble(HydroParams params_, DataArray Udata_) :
			...
			static void apply(HydroParams params,
					...
					int nbCells)
			{
				MakeBoundariesFunctor3D_RisingBubble<faceId> functor(params, Udata);
				Kokkos::parallel_for(nbCells, functor);
			}
