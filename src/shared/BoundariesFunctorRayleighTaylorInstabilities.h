...
namespace euler_kokkos
{
	...
	template <FaceIdType faceId>
		class MakeBoundariesFunctor2D_RayleighTaylorInstabilities : all_regime::HydroBaseFunctor2D
	{
		public:
			MakeBoundariesFunctor2D_RayleighTaylorInstabilities(HydroParams params_, DataArray2d Udata_) :
			...
			static void apply(HydroParams params,
					...
					int nbCells)
			{
				MakeBoundariesFunctor2D_RayleighTaylorInstabilities<faceId> functor(params, Udata);
					...
					if (faceId == FACE_YMIN)
					{
						...
						if(i >= imin && i <= imax)
						{
							for (int j=jmin+ghostWidth-1; j>=jmin; --j)
							{
								...
								const real_t gamma0 =Udata(i, j0, IH)>ZERO_F? params.settings.gamma0:params.settings.gamma1;
	...
	template <FaceIdType faceId>
		class MakeBoundariesFunctor3D_RayleighTaylorInstabilities : all_regime::HydroBaseFunctor3D
	{
		public:
			MakeBoundariesFunctor3D_RayleighTaylorInstabilities(HydroParams params_, DataArray Udata_) :
			...
			static void apply(HydroParams params,
					...
					int nbCells)
			{
				MakeBoundariesFunctor3D_RayleighTaylorInstabilities<faceId> functor(params, Udata);
				Kokkos::parallel_for(nbCells, functor);
			}
