        ...
        if (faceId == FACE_YMIN)
        {
            ...
            if(i >= imin && i <= imax)
            {
                for (int j=jmin+ghostWidth-1; j>=jmin; j--)
                {
                    ...
                    const real_t T_R=q_R[IP]/(q_R[ID]*Rstar);
                    const real_t T_L = T_bot * TWO_F-T_R;
                    HydroState q_L;
                    q_L[ID] = q_R[IP]/Rstar/T_L;
        ...
        if (faceId == FACE_YMAX)
        {
            ...
            if(i >= imin && i <= imax)
            {
                for (int j=ny+ghostWidth; j<=ny+2*ghostWidth-1; j++)
                {
                    ...
                    const real_t T_L=q_L[IP]/(q_L[ID]*Rstar);

                    const real_t T_R = TWO_F * T_top - T_L;

                    HydroState q_R;
                    q_R[ID] = q_L[IP]/ Rstar /T_R;
